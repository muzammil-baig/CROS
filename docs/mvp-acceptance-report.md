...
Root cause: hazard creation called `emit_client_event` as the previously quarantined actor/device, so the event bus correctly rejected the state change at `_verify_device_signature` with `DEVICE_REVOKED`. The campaign was incorrectly continuing under the revoked identity instead of handing off to a surviving responder/gateway identity. The fix preserves revocation enforcement, requires the active caller device to pass `default_device`, requires a declared handoff source to be a revoked device, and records `handoff_from_device_id`, `handoff_event_id`, and `handoff_reason` alongside the active origin actor/device in the hazard event for audit provenance.

The previously captured direct run `RUN-DEDUP-FIX-c1249edc631341d99c754268f00c2f07` proves corrected deduplication, hazard creation, route recomputation, deterministic fallback, allocation, recommendation, and Safety-Critic blocking. The later `RUN-HITL-bc936c75dd1e45a083b62b0e339f3327` proves approval, mission dispatch, and scoped communication delivery/ack, but it is a separate run. Final execution `RUN-FINAL-7f5f01a546b543259314100f7be93f03` created request `REQ-F936936BA3DC4D2E` with applied PostgreSQL sync, deterministic fallback, and a same-run recommendation, but hazard creation returned `422 EVENT_REJECTED / DEVICE_REVOKED`; explicit verification/priority reads returned `403` for the citizen role as designed. The final requested single correlation-scoped chain therefore remains unproven and MVP readiness is not claimed.
...
- Final requested one-run campaign: **BLOCKED/NOT PROVEN**. `RUN-FINAL-7f5f01a546b543259314100f7be93f03` created `REQ-F936936BA3DC4D2E` with PostgreSQL sync and deterministic fallback, but hazard creation returned `422 DEVICE_REVOKED`; verification/priority reads returned `403` for the citizen role, and no same-run HITL/mission/communication/audit closure was captured. Existing separate evidence remains: `RUN-DEDUP-FIX-c1249edc631341d99c754268f00c2f07` for hazard/route/critic and `RUN-HITL-bc936c75dd1e45a083b62b0e339f3327` for approval/dispatch/communication.
